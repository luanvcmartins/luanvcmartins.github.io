...
[famously cast away from the rest of Skyrim](https://elderscrolls.fandom.com/wiki/College_of_Winterhold_(Skyrim)#Distrust){target="_blank"}.
...
- The `Dragonborn DLC` content is on a new map, accessible by ship only. As a result, it displays a
- In contrast, the `Dawnguard DLC` stays on the original map, sharing much with the rest of the game.
...
- There is a distinctive cluster near `Falkreath` related to the  _Sinding's quest_ (Ill Met by Moonlight). It's
  interesting to see it displayed so distinctively in the network, as when I first played it many years ago, I also did
  find it distinctive, as it seemed like there was much more effort put into it than the rest of the quests in the
  game.
